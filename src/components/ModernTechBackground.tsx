...
const ModernTechBackground = () => {
  ...
  useEffect(() => {
    ...
    // Cores primárias para tema verde
    const primaryColor = "rgba(34, 197, 94, 0.4)";
    const accentColor = "rgba(34, 197, 94, 0.7)";
    const glowColor = "rgba(34, 197, 94, 0.2)";

    // Partículas conectadas com efeito neural
    class NeuralNode {
      x: number;
      y: number;
      vx: number;
    ...
    const drawHexagon = (centerX: number, centerY: number, size: number) => {
      ...
      // Adicionar pontos brilhantes ocasionais
      if (Math.random() < 0.02) {
    ...
    const animate = () => {
      ...
      // Atualizar e desenhar nós neurais
      ...
      // Atualizar e desenhar código hexadecimal
  ...
  return (
    <canvas
      ...
      style={{ background: "linear-gradient(to bottom, #000000, #0a0a0a, #050505)" }}
    />
  );
};

export default ModernTechBackground;
